...
WORKFLOW_SKILLS = {
    "research_reading_workflow": "research_reading_workflow_v2",
    "exam_preparation_workflow": "exam_preparation_workflow_v2",
    "course_project_workflow": "course_project_workflow_v2",
}
...
def test_student_workflow_packages_load_pin_and_remain_inactive() -> None:
    ...
    for name in WORKFLOW_SKILLS:
        package = registry.get(name, "0.1.0")
        pin = registry.pin(name, "0.1.0")
...
def test_student_workflow_commands_and_aliases_are_unique() -> None:
    ...
    for name in WORKFLOW_SKILLS:
        invocation = registry.get(name, "0.1.0").manifest.invocation
...
def test_exam_question_papers_cannot_contain_answers_or_explanations() -> None:
    schema = _schema("exam_preparation_workflow_v2", "output.json")
...
def test_defense_questions_cannot_contain_hidden_reference_points() -> None:
    schema = _schema("course_project_workflow_v2", "output.json")
