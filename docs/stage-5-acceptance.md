...
预算/权限/审计、事务式 reload/回滚、PostgreSQL active pointer，以及四个 `0.1.0` Skill：
...
结果：Ruff format/check 通过；mypy 覆盖 94 个源文件；后端全量 pytest 为
`625 passed, 45 skipped`；Web lint/typecheck、Vitest `16 passed` 和 production build 通过；
