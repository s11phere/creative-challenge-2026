...
继续编写不接生产入口的 fake 契约。阶段 3 已按 ADR-010 终止但质量门禁未通过，且该 Port 没有 PostgreSQL/Worker
...
**暂缓情况（2026-07-19）**：已复核并按依赖约束跳过。当前仓库只有阶段 2 的六张业务表，
尚无阶段 4 的 Conversation、AgentRun 和 Evidence 领域/ORM/API 契约。Step 1 已提供纯领域
`CheckpointStore` Port、`RunCheckpoint` 和恢复前置校验，但本步骤不得据此新建平行 AgentRun
...
**暂缓情况（2026-07-19）**：已复核并跳过。阶段 2 摄入闭环、阶段 3 RetrievalStore 和
...
**暂缓情况（2026-07-19）**：已复核并跳过。Step 5/6 未解阻，且 ADR-007 或等价 SSE、
...
**完成情况（2026-07-19）**：通用可执行部分已完成。`FileSystemSkillRegistry.reload()` 在
进程锁内先验证完整受信目录扫描和临时快照，再一次性发布新不可变版本；任一缺失文件、
摘要冲突或兼容失败均不产生部分注册。注册、激活、回滚、固定和恢复兼容校验使用同一可重入
锁；活动指针只影响后续 `pin`，既有固定版本保持旧摘要。激活/回滚失败保持原活动版本，
生命周期事件记录版本、摘要和前一活动版本，不记录包正文或路径。

恢复兼容校验会重新读取旧包并检查完整摘要、checkpoint schema、verified 状态及所需 Tool
版本。新增 6 个测试覆盖批量加载全有或全无、升级中旧 pin、原子回滚、冲突/回滚失败、
旧检查点恢复、Tool/schema 不兼容、磁盘篡改和并发激活/固定。由于 Step 5 持久化事实源缺失，
Registry 不提供旧版本删除 API；AgentRun/Checkpoint/审计引用查询、进程重启恢复和受控清理
