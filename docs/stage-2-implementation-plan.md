...
阶段 0 已通过 `docs/stage-0-acceptance.md` 交接，manifest 为 `frozen` 且范围为 `internal_team_only`。阶段 2 Step 9 仍必须在隔离环境中执行，不能以合成数据或工程完成度替代。当前约束如下：
...
**状态**：已完成。不存在会改变实体字段、幂等身份或删除语义的未决技术问题；ID、版本、发布、任务和删除规则以 ADR-005 为准。阶段 0 已通过冻结记录交接；阶段 2 Step 9 正式质量验收仍是独立门禁，不属于本步骤的技术决策未决项。
...
**状态**：已于 2026-07-18 完成。ADR-005 已接受；P0 格式边界保持 Markdown、TXT、可复制文本 PDF；扫描件/OCR 保持 P2；阶段 1 diagnostic actor 日志差异按 ADR-005 接受为进入风险，PostgreSQL 持久任务状态、Redis 队列、trace ID 和死信证据作为替代诊断依据。阶段 0 已冻结；Step 2 的工程完成不替代阶段 2 Step 9 正式质量验收。
...
- 新增 `packages/infrastructure/src/infrastructure/parsers/` 包：
  ...
  - **`PdfParser`**：基于固定版本 `PyMuPDF==1.28.0` 的 `Page.get_text("text")`，每页输出 `RAW_TEXT` 节点附带 1-based 页码；全页无提取文本时返回 `scanned_pdf` 错误码而非静默空结果。
...
- 新增依赖：`markdown-it-py>=3.0`、`PyMuPDF==1.28.0`。测试夹具可使用锁定的
  `pypdf==6.14.2` 生成最小 PDF，但不参与生产提取。PDF evidence 的提取协议固定在
  `cases/evals/corpus/v0/PDF-EXTRACTION.md`，避免版本变化导致定位结果漂移。
...
| 内部冻结范围被误当作正式质量结论 | 退出声明引用真实语料指标 | 成功率口径显式标注样本来源；Stage 0 冻结不替代 Step 9 正式验收 |
