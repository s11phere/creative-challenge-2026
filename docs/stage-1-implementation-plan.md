...
阶段 0 已于 2026-07-28 按 `docs/stage-0-acceptance.md` 正式完成，并冻结为
`internal_team_only`。本节以下内容保留为阶段 1 启动时（2026-07-17）的历史门禁记录，
不代表当前状态：

- 当时的 `../cases/evals/corpus/v0/manifest.yaml` 状态为 `draft_pending_license_review`。
- 当时评测集结构和 Evidence 定位已机械校验，但许可证和人工标注复核尚未完成。
- 当时私有、受限或授权未确定的语料不得进入仓库、CI、公开截图或外部模型调用。

当时采用“条件式启动”：阶段 1 的无语料工程工作可以并行开始，但项目阶段状态保持在阶段 0，直到
授权复核、人工标注复核和版本冻结完成。当前仍应只使用代码内新建的最小确定性数据或 manifest 明确
允许 `repository_fixture` 的公开 fixture；内部语料读取必须校验哈希并遵守 `allowed_uses`、敏感度和
外部 Provider 策略。

阶段 0 收尾当时建议单独安排 2-5 个人工复核人日；该工作现已由冻结记录关闭。它不改变阶段 2 Step 9、
阶段 3 正式退出或阶段 4 启动所需的独立质量门禁。
