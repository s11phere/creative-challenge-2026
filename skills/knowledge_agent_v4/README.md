# knowledge_agent 0.4.0

This is the provisional generic-loop candidate package. It is not the default active version until
the staged rollout enables it. It runs a bounded LLM decision loop over the server-registered
read-only Tools `inspect_retrieval 1.0.0` and `grounded_qa 1.0.0`. Grounded QA remains the answer,
Citation, persistence, and SSE authority.
