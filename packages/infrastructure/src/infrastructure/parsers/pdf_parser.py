...
def _order_blocks(blocks: list[list[_Span]]) -> list[list[_Span]]:
    """Order blocks column-first, then top-to-bottom within each column.

    PyMuPDF's block order follows extraction/insertion order, which is not a
    reading-order contract. Nearby x positions are kept in one column so
    indentation does not reorder a single-column paragraph.
    """
    positioned = [
        (
            min(float(span["bbox"][0]) for span in block),
            min(float(span["bbox"][1]) for span in block),
            block,
        )
        for block in blocks
    ]
    column_starts: list[float] = []
    for x0, _, _ in sorted(positioned, key=lambda item: (item[0], item[1])):
        if not column_starts or x0 - column_starts[-1] > _COLUMN_X_EPS:
            column_starts.append(x0)

    def key(item: tuple[float, float, list[_Span]]) -> tuple[int, float, float]:
        x0, y0, _ = item
        column = min(range(len(column_starts)), key=lambda index: abs(x0 - column_starts[index]))
        return column, y0, x0

    return [block for _, _, block in sorted(positioned, key=key)]
...
def _page_visual_lines(page: fitz.Page, page_no: int) -> list[_VisualLine]:
    ...
    """
    blocks = _order_blocks(_iter_blocks(page))
    if not blocks:
        return []
        ...
        if _is_list_start(line):
            ...
            while j < len(lines):
                ...
                if _indent_matches(nxt, indent) and _gap_ok(lines[j - 1], nxt):
                    ...
                    continue
                break
            emit(StructNodeType.LIST_ITEM, "\n".join(texts).strip(), 0, start, j)
            i = j
            continue
