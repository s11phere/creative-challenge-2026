...
**阶段 0 已按内部组间受控范围正式完成；阶段 2 Step 0-8 与阶段 3 Step 0-10 的工程实现已完成并通过本地验证。阶段 2 Step 9 正式质量验收和阶段 3 正式 holdout 质量验收尚未完成。阶段 5 通用 Agent Runtime/Skill 基础已并行通过审查。**
...
| 阶段 3 ✅ 工程 Step 0-10 | PostgreSQL FTS/pgvector 检索、加权 RRF、上下文扩展、可选 Reranker、Space/版本安全边界、检索 API、版本化离线评测、集成验收与移交文档；正式质量门禁仍待阶段 2 Step 9 与阶段 3 配置/模型验收 |
...
[Stage 0 验收记录](docs/stage-0-acceptance.md)。不要直接运行 holdout；必须先按验收记录中的正式完成清单完成阶段 2 Step 9、真实模型
